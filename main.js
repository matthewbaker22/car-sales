...
container.innerHTML = `<h2>Weekly Sales Report</h2>`

salesByWeek.forEach(sales => {
    container.innerHTML += `<hr><div><strong>${sales.sales_agent.first_name} ${sales.sales_agent.last_name}</strong></div><br>`

    for (const entry of Object.entries(sales.vehicle)) {
        container.innerHTML += `<div>${entry[0]}: ${entry[1]}</div>`
    }

    container.innerHTML += `<br><div><strong>Profit: $${sales.gross_profit}</strong></div>`
})